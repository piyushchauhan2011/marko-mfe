...
async function startServer() {
  ...
  if (await isPortInUse(port)) {
    console.log(`Port ${port} already in use; skipping experiences-discovery server restart.`);
    return null;
  }
...
function watchAndSync() {
  ...
  const scheduleBundle = () => {
    ...
    timer = setTimeout(() => {
      buildClientBundle().catch((error) => {
        console.error('Failed to rebuild experiences-discovery client bundle.', error);
      });
    }, 150);
  };
