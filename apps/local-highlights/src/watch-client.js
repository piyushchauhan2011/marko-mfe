...
async function startServer() {
  ...
  if (await isPortInUse(port)) {
    console.log(`Port ${port} already in use; skipping local-highlights server restart.`);
    return null;
  }
...
function watchAndSync() {
  ...
  fs.watch(appDir, { recursive: true }, (eventType, filename) => {
    if (!filename || !/\.(js|jsx|json)$/.test(filename)) {
      return;
    }
    scheduleBundle();
  });

  console.log(`Watching ${appDir} for React client updates...`);
}
